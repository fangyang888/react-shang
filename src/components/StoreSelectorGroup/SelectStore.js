...
const SelectStore = props => {
    const { storeTypes, storeShowNotOnline, isPermission, style } = props;
    ...
    const [selectParams, setSelectParams] = useState({ checkedList: [], indeterminate: true, checkAll: false })
    ...
    const callbackCity = useCallback(async () => {
        ...
        const list = await getCity(params);
        if (list) {
            list.forEach(({ name, zip }) => {
                const obj = { label: name, value: zip, children: [] };
                options.push(obj);
                defaultList.push(zip)
            })
            setCityOptions(options);
            // setDefaultCity(defaultList);
            setStoreOptions(options);
        }

    }, [storeTypes, storeShowNotOnline])
    useEffect(() => {
        callbackCity();
    }, [callbackCity])
    useEffect(() => {
    ...
    const getCityToStore = async value => {
        ...
        if (store && store.children.length === 0) {
            const params = {
            ...
            store.children = storeList.map(item => Object.assign(item, { label: item.abbr_name, value: item.code }));
            setStoreOptions([...storeOptions])
            setStoreList(storeList)
            setStoreMap(c => Object.assign(c, { [value]: [] }))
        } else {
            setStoreList(store.children)
        }
    }
    const onCheckAllChange = e => {
        const checked = e.target.checked;
        const list = storeList.map(({ code }) => code);
        ...
        <div>
            ...
            </Radio.Group>
            {
                type === 1 && <Row style={{ 'lineHeight': '3em' }}>
                    <Checkbox.Group options={cityOptions} defaultValue={defaultCity} onChange={(v) => changeCity(v)} />
                </Row>
            }
            {
                type === 2 && <Row >
